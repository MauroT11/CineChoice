...
export default function WatchlistTVCard({Series, Genre, Genres, onRemove}) {
    return (
        <div className="flex flex-col items-center min-h-full w-full max-w-[2000px] mx-auto px-4">
            {Genres ? (
              <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-8 text-center gap-3 mb-6">
                {Genres.map((genre) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 w-full max-w-[1800px] px-6 mx-auto place-items-center">
              {Series.map((serie, index) => (
                <div key={serie.id} 
                  ...
                  <figure className="relative h-[375px]" onClick={() => document.getElementById(`my_modal_${serie.id}`).showModal()}>
                    <Image 
