  ...
  // console.log(movies)    
    ...
    return (
        <div className="flex flex-col items-center min-h-full">
            {Genre ? (
              <h1 className="text-5xl font-bold my-4">Popular {Genre} Movies</h1>
            ) : (
            ...
            {Genres ? (
              <div className="grid grid-cols-10 text-center gap-2 mb-4">
                {Genres.map((genre) => (
                  <a href={`/movies/genre/${genre.id}`} key={genre.id} className="border-2 border-secondary rounded-3xl px-2 hover:border-accent hover:bg-accent hover:text-base-100 text-lg">{genre.name}</a>
                ))}
              </div>
            ) : (
              null
